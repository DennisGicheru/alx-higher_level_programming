...
by a div and returns all results to 2 decimal places
"""
...
def matrix_divided(matrix, div):
    ...
    if type(matrix) is not list or not all((type(i) is list)for i in matrix)\
        or not all((isinstance(e, (int, float))for e in i)for i in matrix)\
            or len(matrix[0]) == 0:
        raise TypeError(
    ...
    if (div is None or not isinstance(div, int) and
